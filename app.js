...
const state = {
  ...
  search: {
    ...
    notFoundDetail: null,      // detail string ready to log if the user clicks "Contact Finance"
    notFoundLogged: false      // becomes true once they've clicked it, for this search
  },
  history: {
    items: [],
    loading: false
...
// ---------------------------------------------------------------
const ADMIN_EMAIL = "moyos@zimnat.co.zw";
function isAdmin(){
  return !!(state.auth.email && state.auth.email.toLowerCase() === ADMIN_EMAIL);
}

function getSignature(){
  try{
    return JSON.parse(localStorage.getItem("pct_signature") || "null") || { name:"", title:"" };
...
async function initSupabase(){
  ...
  try{
    ...
    state.auth.session = sessionData?.session || null;
    state.auth.email = sessionData?.session?.user?.email || null;

    state.sb.auth.onAuthStateChange((_event, session) => {
      state.auth.session = session || null;
      state.auth.email = session?.user?.email || null;
      updateConnBadge();
      render();
    });
  }catch(e){
...
async function logAudit(eventType, detail){
  if(!state.sb || !state.auth.email) return;
  try{
    ...
    // Best-effort — never block the user's actual action on audit logging failing.
  }
}

// A search coming up empty doesn't log itself — it just remembers what it
// would say, so the "Contact Finance" button can send it only if the user
// actually wants Finance notified (avoids logging every typo/near-miss).
function logAuditNotFound(detail){
  state.search.notFoundDetail = detail;
  state.search.notFoundLogged = false;
}

async function contactFinanceAboutSearch(){
  if(!state.search.notFoundDetail || state.search.notFoundLogged) return;
  await logAudit("not_found", state.search.notFoundDetail);
  state.search.notFoundLogged = true;
  toast("Finance has been notified.");
  render();
}
...
async function handleSignIn(email, password){
  ...
  if(error){
    a.loginError = error.message;
    render();
    return;
  }
  a.session = data.session;
  a.email = data.user?.email || email;
  logAudit("sign_in", null);
  state.view = "confirm";
  render();
}
...
function updateConnBadge(){
  ...
  const authEl = document.getElementById("sideAuthStatus");
  if(authEl){
    if(state.auth.email){
      authEl.innerHTML = `<div class="auth-status-email">${escapeHtml(state.auth.email)}</div><button class="auth-signout-link" id="sideSignOutBtn">Sign out</button>`;
      const btn = document.getElementById("sideSignOutBtn");
...
function subtractMonthsFromIsoDate(isoStr, n){
  // isoStr is "YYYY-MM-DD". Parsed as local (not UTC) to avoid timezone day-shift.
...
// Router
// ---------------------------------------------------------------
const ADMIN_ONLY_VIEWS = ["upload", "history", "settings", "audit"];
function setView(v){
  if(ADMIN_ONLY_VIEWS.includes(v) && !isAdmin()){
    toast("That section is restricted to Finance.", true);
    return;
  }
  state.view = v;
...
function render(){
  ...
  // Stage 1 — not pointed at a database yet.
  if(!state.connected){
  ...
  // Stage 2 — connected, but not signed in.
  ...
  // Stage 3 — signed in, but hasn't given a name for this browser yet.
  if(!getSignature().name){
    if(navEl) navEl.style.display = "none";
    root.innerHTML = renderNameView();
    bindViewEvents();
    return;
  }
  ...
  if(ADMIN_ONLY_VIEWS.includes(state.view) && !isAdmin()){
    state.view = "confirm"; // defensive fallback — the click handler already blocks this normally
  }
  ...
  else if(state.view === "audit") root.innerHTML = renderAuditView();
  else if(state.view === "settings") root.innerHTML = renderSettingsView();
  bindViewEvents();
}

function renderConnectView(){
  const motifSrc = (typeof ZIMNAT_TRIANGLE_MOTIF_URI !== "undefined") ? ZIMNAT_TRIANGLE_MOTIF_URI : "";
  const cfg = getConfig();
  return `
    <div class="login-shell">
      ${motifSrc ? `<img class="login-bg-motif top" src="${motifSrc}" alt="">` : ""}
      ${motifSrc ? `<img class="login-bg-motif bottom" src="${motifSrc}" alt="">` : ""}
      <div class="login-card">
        <img class="login-logo" src="${ZIMNAT_LOGO_DATA_URI}" alt="Zimnat General Insurance">
        <h1>Connect to the register</h1>
        <p>This device needs to be pointed at the register's database once. Ask whoever set this up for the two values below.</p>
        ${state.connError && state.connError !== "not-configured" ? `<div class="login-error">${escapeHtml(state.connError)}</div>` : ""}
        <div class="field">
          <label>Supabase project URL</label>
          <input type="text" id="connectUrl" placeholder="https://xxxxx.supabase.co" value="${escapeHtml(cfg?.url || "")}">
        </div>
        <div class="field">
          <label>Supabase anon public key</label>
          <input type="text" id="connectKey" placeholder="sb_publishable_..." value="${escapeHtml(cfg?.key || "")}">
        </div>
        <button class="btn btn-primary" id="connectBtn" style="width:100%; justify-content:center;">Connect</button>
      </div>
    </div>
  `;
}
  ...
  return `
    <div class="login-shell">
      ...
      <div class="login-card">
        ...
        <p>What name should show on confirmations and replies you send from this device?</p>
        <div class="field">
          <label>Your name</label>
          <input type="text" id="nameInput" placeholder="e.g. S. Moyo">
        </div>
        <div class="field">
          <label>Title / Department</label>
          <input type="text" id="titleInput" placeholder="e.g. Finance Department" value="Finance Department">
        </div>
        <button class="btn btn-primary" id="nameBtn" style="width:100%; justify-content:center;">Continue</button>
      </div>
    </div>
  `;
}
...
function renderLoginView(){
  ...
  return `
    <div class="login-shell">
      ...
      <div class="login-card">
        <img class="login-logo" src="${ZIMNAT_LOGO_DATA_URI}" alt="Zimnat General Insurance">
        <h1>Sign in to the Premium Register</h1>
        <p>Access is limited to Finance and Claims staff with an account. Contact whoever manages this tool if you need one.</p>
        ${a.loginError ? `<div class="login-error">${escapeHtml(a.loginError)}</div>` : ""}
        <div class="field">
          <label>Email</label>
          <input type="text" id="loginEmail" placeholder="name@zimnat.co.zw" value="${escapeHtml(a.loginEmail || "")}">
        </div>
        <div class="field">
          <label>Password</label>
          <input type="password" id="loginPassword" placeholder="••••••••">
        </div>
        <button class="btn btn-primary" id="loginBtn" style="width:100%; justify-content:center;" ${a.loginLoading ? "disabled" : ""}>
          ${a.loginLoading ? '<span class="spinner" style="border-color:rgba(255,255,255,0.4); border-top-color:#fff;"></span> Signing in…' : "Sign in"}
        </button>
      </div>
    </div>
  `;
...
function renderConfirmView(){
  ...
  if(s.loading){
    ...
    const canContact = !!s.notFoundDetail;
    resultsHtml = `
      <div class="empty-state">
        <div class="es-icon">🔍</div>
        <h3>No record for "${escapeHtml(s.term)}"</h3>
        <p>We couldn't find a matching premium. Double-check the spelling or reg number, or ask Finance to take a look.</p>
        ${s.note ? `<p class="step-note" style="margin-top:10px;">${escapeHtml(s.note)}</p>` : ""}
        ${canContact ? `
          <div style="margin-top:18px;">
            ${s.notFoundLogged
              ? `<span class="tag">✓ Finance notified</span>`
              : `<button class="btn btn-primary" id="contactFinanceBtn">Contact Finance about this</button>`}
          </div>
        ` : ""}
      </div>`;
    ...
    <div class="view">
      ...
      <div class="panel">
        ...
        <div class="field-row" style="margin-top:14px;">
          <div class="field">
            <label>Month${s.coverStart ? ' <span class="tag warn" style="margin-left:4px;">unused</span>' : ""}</label>
            <select id="searchMonth" ${s.coverStart ? "disabled" : ""}>
          ...
          <div class="field">
            <label>Year</label>
            <select id="searchYear" ${(s.month === "" || s.coverStart) ? "disabled" : ""}>
        ...
        <div class="step-note">${s.coverStart
          ? `Searching payments dated up to ${COVER_LOOKBACK_MONTHS} months before this cover start date, through to the cover start date itself — Month/Year is ignored while a cover start date is set.`
          : "Optional — enter the claim's cover start date to search only payments made up to 2 months before it, through to the cover start date."}</div>
        ${resultsHtml}
      </div>
      ${certHtml}
    </div>
...
const COVER_LOOKBACK_MONTHS = 2; // premium expected up to 2 months before cover start

async function runSearch(term){
  state.search.term = term;
  state.search.selected = null;
  state.search.note = null;
  state.search.notFoundDetail = null;
  state.search.notFoundLogged = false;
  ...
  const { month, year, currency, category, coverStart } = state.search;

  try{
    if(coverStart){
      // Cover start date given — this drives the search directly: only look at
      // payments dated up to 2 months before cover start, through cover start
      // itself. Month/Year filters are ignored in this mode (see UI note).
      const rangeStart = subtractMonthsFromIsoDate(coverStart, COVER_LOOKBACK_MONTHS);

      let q = state.sb.from("bord_entries").select("*")
        .ilike("client_raw", `%${cleanTerm}%`)
        .gte("date_paid", rangeStart)
        .lte("date_paid", coverStart);
      ...
      if(data && data.length){
        ...
        state.search.note = `Searched payments from ${fmtDate(rangeStart)} to ${fmtDate(coverStart)} (up to ${COVER_LOOKBACK_MONTHS} months before cover start).`;
      } else {
        // Nothing in the expected window — check for a late payment (after cover
        // start) so that case can still be flagged rather than looking like "no record".
        let q2 = state.sb.from("bord_entries").select("*")
          .ilike("client_raw", `%${cleanTerm}%`)
          .gt("date_paid", coverStart);
        if(currency) q2 = q2.eq("currency", currency);
        if(category) q2 = q2.eq("category", category);
        const { data: lateData, error: lateErr } = await q2.order("date_paid", { ascending:true }).limit(10);
        if(lateErr) throw lateErr;

        if(lateData && lateData.length){
          state.search.results = lateData;
          state.search.note = `No payment found from ${fmtDate(rangeStart)} to ${fmtDate(coverStart)} — found one paid after the cover start date instead.`;
        } else {
          state.search.results = [];
          state.search.note = `Searched ${fmtDate(rangeStart)} to ${fmtDate(coverStart)}, or afterward.`;
          logAuditNotFound(`"${cleanTerm}" — no match ${fmtDate(rangeStart)}–${fmtDate(coverStart)} or after${currency?" · "+currency:""}${category?" · "+category:""}`);
        }
      }
    } else if(!month){
      // No cover date and no month picked — search the whole register, most recent first.
      ...
      state.search.note = null;
      if(!data || !data.length){
        logAuditNotFound(`"${cleanTerm}" — no match across all periods${currency?" · "+currency:""}${category?" · "+category:""}`);
      }
    } else {
      // Month picked (no cover date) — search that bord first, then expand outward until a match turns up.
      const offsets = [0];
      ...
      if(found && foundOffset !== 0){
        ...
        state.search.note = `Searched ${month} ${year} and ${SEARCH_EXPAND_RANGE} months either side (${checkedPeriods[checkedPeriods.length-2]} to ${checkedPeriods[checkedPeriods.length-1]}).`;
        logAuditNotFound(`"${cleanTerm}" — no match near ${month} ${year}${currency?" · "+currency:""}${category?" · "+category:""}`);
      } else {
        state.search.note = null;
      }
    }
  ...
  try{
    const { data: uploadRow, error: uploadErr } = await state.sb
      ...
      .insert({
        ...
        uploaded_by: state.auth.email || null
      })
      .select()
      .single();
    ...
    logAudit("upload", `${period} · ${currency} · ${u.category} · ${u.rows.length} rows · ${u.fileName || ""}`);
    resetUpload();
  }catch(err){
    ...
    body = h.items.map(it => `
      <div class="hist-row">
        <div>
          ...
          <div class="hmeta">${it.row_count} polic${it.row_count===1?"y":"ies"} · ${escapeHtml(it.source_file || "")} · uploaded ${fmtDate(it.uploaded_at)}${it.uploaded_by ? " by " + escapeHtml(it.uploaded_by) : ""}</div>
        </div>
...
async function deleteUpload(id){
  ...
  const target = state.history.items.find(it => it.id === id);
  ...
  logAudit("delete_upload", target ? `${target.period} · ${target.currency} · ${target.row_count} rows` : id);
  loadHistory();
}
...
const AUDIT_LABELS = {
  sign_in: "Signed in",
  upload: "Uploaded a bord",
  delete_upload: "Removed a bord",
  not_found: "Search not found — needs follow-up"
};
...
function renderAuditView(){
  ...
  let body = "";
  if(a.loading){
    body = `<div class="empty-state"><div class="spinner" style="border-color:rgba(27,42,57,0.2); border-top-color:var(--navy); margin:0 auto 12px;"></div><p>Loading audit trail…</p></div>`;
  } else if(!a.items.length){
    body = `
      <div class="empty-state">
        <div class="es-icon">🕒</div>
        <h3>No activity recorded yet</h3>
        <p>Sign-ins, uploads, removed bords, and searches that came up empty will be listed here, with who and when.</p>
      </div>`;
  } else {
    body = a.items.map(it => {
      const isNotFound = it.event_type === "not_found";
      return `
      <div class="hist-row" ${isNotFound ? 'style="border-color:#F0D9B5; background:#FFFBF3;"' : ""}>
        <div>
          <div class="hname">${isNotFound ? '<span class="tag warn" style="margin-right:6px;">Needs help</span>' : ""}${escapeHtml(AUDIT_LABELS[it.event_type] || it.event_type)}</div>
          <div class="hmeta">${escapeHtml(it.user_email || "unknown")}${it.detail ? " · " + escapeHtml(it.detail) : ""} · ${fmtDate(it.created_at)}</div>
        </div>
      </div>
    `;
    }).join("");
  ...
  const notFoundCount = a.items.filter(it => it.event_type === "not_found").length;

  return `
    <div class="view wide">
      <div class="page-head">
        <h1>Audit trail</h1>
        <p>Who has signed in, what they've uploaded or removed, and searches that came up empty — most recent first.</p>
      </div>
      ${notFoundCount ? `<div class="conn-banner pending">${notFoundCount} search${notFoundCount>1?"es":""} came up empty and may need your help.</div>` : ""}
      <div class="panel">${body}</div>
    </div>
  `;
}
...
async function loadAudit(){
  ...
  if(error){
    toast("Couldn't load audit trail: " + error.message, true);
  } else {
    state.audit.items = data || [];
  }
  render();
}
    ...
    <div class="view">
      ...
      <div class="panel">
        ...
        <ol class="setup-steps">
          ...
          <li>Add each person who should have access: <strong>Authentication → Users → Add user</strong>, enter their email and a password, and tick <strong>Auto Confirm User</strong> so they can sign in immediately. This list is exactly who can access the register — add or remove people here.</li>
          <li>Share this tool's link (and the same URL + key) with everyone you added — they'll each sign in with the email and password you set for them.</li>
        </ol>
        <div class="sql-box" id="sqlBox">${escapeHtml(SETUP_SQL)}</div>
        <div style="margin-top:10px;">
          <button class="btn btn-ghost btn-sm" id="copySqlBtn">Copy SQL</button>
        </div>
        <p class="step-note">Every sign-in, upload, and bord removal is recorded with the person's email in <strong>Audit trail</strong> — that's your access log alongside the user list itself.</p>
      </div>
    </div>
  `;
}
...
-- Only signed-in users (created in Authentication > Users) can read anything,
-- and only moyos@zimnat.co.zw can upload, remove bords, or view the audit trail.
-- Change the email below if the Finance admin account changes.
...
-- ---------------------------------------------------------------
-- MIGRATION: already ran an earlier version of this schema?
-- Run just this block instead of everything above.
-- ---------------------------------------------------------------
...
-- alter table audit_log enable row level security;
--
-- -- If audit_log already existed without 'not_found' allowed:
...
--
-- -- Replace any older open policies with the admin-restricted ones below
...
function bindViewEvents(){
  ...
  if(loginEmail) loginEmail.addEventListener("input", (e)=>{ state.auth.loginEmail = e.target.value; });
  const doLogin = ()=>{
  ...
  if(connectBtn){
    connectBtn.addEventListener("click", async ()=>{
      ...
      await initSupabase();
      render();
      if(state.connected) toast("Connected.");
    });
  }
  ...
  if(nameBtn){
    nameBtn.addEventListener("click", ()=>{
      ...
      if(!name){ toast("Enter your name to continue.", true); return; }
  ...
  if(coverStartInput) coverStartInput.addEventListener("change", (e)=>{
    state.search.coverStart = e.target.value;
    if(state.search.term && state.search.term.trim().length >= 2) runSearch(state.search.term);
    else render();
  });
  const coverEndInput = document.getElementById("coverEndInput");
  if(coverEndInput) coverEndInput.addEventListener("change", (e)=>{
    state.search.coverEnd = e.target.value;
    render(); // cover end is display-only (not part of the query), no re-search needed
  });
  ...
  const contactFinanceBtn = document.getElementById("contactFinanceBtn");
  if(contactFinanceBtn) contactFinanceBtn.addEventListener("click", contactFinanceAboutSearch);
  ...
  // ---- Audit trail view ----
  if(state.view === "audit" && !state.audit.items.length && !state.audit.loading){
    loadAudit();
  }
  ...
  if(clearConnBtn){
    clearConnBtn.addEventListener("click", ()=>{
      ...
      state.auth.session = null; state.auth.email = null;
      updateConnBadge();
      render();
    });
  }
