...
function render(){
  ...
  const showingLogin = state.connected && !state.auth.session && state.view !== "settings";
  if(navEl) navEl.style.display = showingLogin ? "none" : "";
  if(showingLogin){
  ...
  return `
    <div class="login-shell">
      ...
      <div class="login-card">
        ...
        <button class="auth-signout-link" id="loginSettingsLink" style="margin-top:16px;">Connection settings</button>
      </div>
    </div>
  `;
}
...
function bindViewEvents(){
  ...
  const loginSettingsLink = document.getElementById("loginSettingsLink");
  if(loginSettingsLink) loginSettingsLink.addEventListener("click", ()=>{ state.view = "settings"; render(); });
