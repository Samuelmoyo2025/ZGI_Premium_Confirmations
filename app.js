  ...
  if(s.loading){
    ...
    resultsHtml = `
      <div class="empty-state">
        ...
        <p>We couldn't find a matching premium. <strong>Please contact Finance for assistance</strong> — they've been notified.</p>
    ...
    if(coverStart){
      ...
      if(data && data.length){
        ...
        if(lateData && lateData.length){
          ...
          logAudit("not_found", `"${cleanTerm}" — no match ${fmtDate(rangeStart)}–${fmtDate(coverStart)} or after${currency?" · "+currency:""}${category?" · "+category:""}`);
        }
      }
    } else if(!month){
      ...
      state.search.note = null;
      if(!data || !data.length){
        logAudit("not_found", `"${cleanTerm}" — no match across all periods${currency?" · "+currency:""}${category?" · "+category:""}`);
      }
    } else {
      ...
      if(found && foundOffset !== 0){
        ...
        logAudit("not_found", `"${cleanTerm}" — no match near ${month} ${year}${currency?" · "+currency:""}${category?" · "+category:""}`);
      } else {
        state.search.note = null;
      }
    }
