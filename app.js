    ...
    if(coverStart){
      ...
      if(data && data.length){
        ...
        if(lateData && lateData.length){
          ...
          state.search.results = [];
          state.search.note = `No payment found from ${fmtDate(rangeStart)} to ${fmtDate(coverStart)}, or afterward. Check the spelling or reg number, or confirm with finance whether this business was written.`;
        }
      }
    } else if(!month){
...
function renderAuditView(){
  ...
  if(a.loading){
    ...
    body = `
      <div class="empty-state">
        ...
        <p>Sign-ins, uploads, and removed bords will be listed here as they happen, with who did them and when.</p>
      </div>`;
  } else {
    body = a.items.map(it => `
      <div class="hist-row">
        <div>
          <div class="hname">${escapeHtml(AUDIT_LABELS[it.event_type] || it.event_type)}</div>
  ...
  return `
    <div class="view wide">
      <div class="page-head">
        <h1>Audit trail</h1>
        <p>Who has signed in and what they've uploaded or removed, most recent first — this is the access log for the register.</p>
      </div>
      <div class="panel">${body}</div>
    </div>
  `;
...
-- Only signed-in users (created in Authentication > Users) can read or write.
-- Anonymous/public access is no longer allowed once these policies are in place.
