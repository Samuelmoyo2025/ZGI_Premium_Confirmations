...
## Files
- `index.html` / `app.js` / `logo-data.js` — the tool itself (the logo is embedded as data directly in `logo-data.js`, so there's no separate image file to lose track of when deploying)
...
5. Deploy `index.html`, `app.js`, and `logo-data.js` together (e.g. to Vercel,
   same as your other finance tools) and share the link with Claims and the
   rest of Finance. Each person who opens it needs to paste the same URL +
   anon key once — Settings shows you exactly what to give them.
